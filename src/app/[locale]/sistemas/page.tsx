...
import { getProductsByCategory, l, getBadgeLabel } from "@/data/products";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "pages.sistemas" });
  return {
    title: `${t("title")}`,
    description: t("subtitle"),
    alternates: buildAlternates(locale, "/sistemas"),
  };
}
          ...
          <div className="mx-auto grid max-w-[1200px] grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {products.map((product, i) => (
              <ScrollReveal key={product.slug} delay={i * 100} direction="up">
                <Link href={{ pathname: "/sistemas/[slug]", params: { slug: product.slug } }} className="group cursor-pointer">
        ...
        <section className="bg-[#f8f8f8] px-6 py-16">
          <div className="mx-auto max-w-[1200px]">
            <div className="mb-10 flex flex-wrap items-end justify-between gap-4">
              ...
              <Link href="/accesorios" className="font-[family-name:var(--font-body)] text-[15px] text-[#A52430] hover:underline">
                {t("relatedLink")}
              </Link>
            </div>

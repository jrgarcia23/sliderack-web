      ...
      <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-start gap-12 md:grid-cols-2 md:gap-20">
              ...
              {["Respuesta en 24-48h", "Asesoramiento técnico", "Sin compromiso"].map(
                (text) => (
                  <div key={text} className="flex items-center gap-2">
                    ...
                    <span className="font-[family-name:var(--font-body)] text-[13px] text-[#999]">
                      {text}
                    </span>
                  </div>
                )
        ...
        <ScrollReveal direction="right">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <input type="text" name="nombre" placeholder="Nombre *" required value={form.nombre} onChange={handleChange} className={inputClasses} />
            <input type="text" name="empresa" placeholder="Empresa" value={form.empresa} onChange={handleChange} className={inputClasses} />
            <input type="email" name="email" placeholder="Email *" required value={form.email} onChange={handleChange} className={inputClasses} />
            <input type="tel" name="telefono" placeholder="Teléfono" value={form.telefono} onChange={handleChange} className={inputClasses} />
            <textarea name="mensaje" placeholder="Mensaje" rows={4} value={form.mensaje} onChange={handleChange} className={inputClasses} />
